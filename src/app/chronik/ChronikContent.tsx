...
function TimelineItem({ event, index }: TimelineItemProps) {
  ...
  // Desktop: left items fade in from left, right items fade from right
  // Mobile: all items always appear on the right of the line (fade in from right)
  const cardVariants = {
...
function EventCard({ event, Icon }: { event: TimelineEvent; Icon: React.ElementType }) {
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-border bg-bg-elevated p-6 transition-shadow duration-300 hover:shadow-xl hover:shadow-primary/10 hover:border-border-accent">
      ...
      <div className="mb-3 flex items-center gap-3">
        <span className="text-3xl font-extrabold tracking-tight text-primary leading-none">
          {event.year}
        </span>
...
export default function ChronikContent() {
  return (
    <main className="min-h-screen bg-bg text-text">
        ...
        <Container>
          <AnimatedSection variants={fadeInUp} className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-6 text-lg leading-relaxed text-text-muted sm:text-xl">
              Von der Gründung 1998 bis heute — fast drei Jahrzehnte Leidenschaft für Räder,
              Logistik und zuverlässigen Service im Raum Stuttgart.
            </p>
          </AnimatedSection>
        </Container>
      </section>
      ...
      <section className="py-20 lg:py-28">
        <Container>
          ...
          {/* Timeline wrapper — relative so the vertical line can be positioned */}
          <div className="relative mx-auto max-w-4xl">
            {/* Vertical connecting line — desktop: centre, mobile: left */}
            <div
              aria-hidden="true"
              className="
            ...
            {/* Events */}
            <div className="flex flex-col gap-10 pl-0 lg:pl-0">
